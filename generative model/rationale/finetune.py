...
if __name__ == "__main__":
    ...
    if args.prop == "3cl_cell,qed,sa" or args.prop == "cell_3cl,qed,sa":
        print('four properties filter')
        args.compare_func = qed_sa_func
    else:
        print('two properties filter')
        args.compare_func = normal_func

    prop_funcs = get_scoring_function(args.prop)
    scoring_function = lambda x : list( zip(*prop_funcs(x)) )

    # prop_funcs = [get_scoring_function(prop) for prop in args.prop.split(',')]
    # def scoring_function(x):
    #     p_3cl, p_cell = prop_funcs[0](x)
    #     p_qed = prop_funcs[1](x)
    #     p_sa = prop_funcs[2](x)
    #     return list(zip(*[p_3cl, p_cell, p_qed, p_sa]))
    # scoring_function = lambda x : list( zip(*[prop_funcs[0](x)]) )
    # scoring_function = lambda x : list( zip(*[func(x) for func in prop_funcs]) )

    with open(args.rationale) as f:
        print("read rationales...")
        rationales = [line.split()[0] for line in f]
    ...
    for epoch in range(args.load_epoch + 1, args.epoch):
        ...
        for total_step, batch in enumerate(dataset):
            ...
            meters = meters + np.array([kl_div, loss.cpu().item(), wacc.cpu().item() * 100, tacc.cpu().item() * 100, sacc.cpu().item() * 100])
