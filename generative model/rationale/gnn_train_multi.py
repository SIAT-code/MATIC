...
for epoch in trange(args.epoch):
    ...
    # dataset = DataFolder2(args.train, args.batch_size)
    dataset = mydataset(args.train)
    mol_loader = DataLoader(dataset, batch_size=args.batch_size, shuffle=True, drop_last=True)
    for batch in dataset:
        ...
        try:
            ...
            loss, kl_div, wacc, tacc, sacc = loss.mean(), kl_div.mean(), wacc.mean(), tacc.mean(), sacc.mean()
            loss.backward()
            # 梯度裁剪 clip_norm二次范数最大值
    ...
    if args.save_iter == -1:
        ...
        torch.save(model_to_save.state_dict(), args.save_dir + "/model." + str(epoch))
        scheduler.step()
        print("learning rate: %.6f" % scheduler.get_lr()[0])