...
class Fingerprint_small(nn.Module):

    def __init__(self, radius, T, input_feature_dim, input_bond_dim,\
            fingerprint_dim, p_dropout):
        super(Fingerprint_small, self).__init__()
...
#         self.mol_attend = nn.ModuleList([nn.Linear(fingerprint_dim, fingerprint_dim) for t in range(T)])
    ...
    def forward(self, atom_list, bond_list, atom_degree_list, bond_degree_list, atom_mask):
        ...
        bond_neighbor = [bond_list[i][bond_degree_list[i]] for i in range(batch_size)]#56 * 6 * 10
        ...
        # atom_feature_expand: tensor(batch_size, max_atom_len, degrees_len, fingerprint_dim*2), eg.(32,178,6,300)
