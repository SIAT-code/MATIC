...
def gen_descriptor_data(smilesList):
    ...
    for i,smiles1 in enumerate(smilesList):
#         if i > 5:
#             print("Due to the limited computational resource, submission with more than 5 molecules will not be processed")
#             break
        try:
            smiles2 = Chem.MolToSmiles(Chem.MolFromSmiles(smiles1), isomericSmiles=True)
        except:
            print('---'+str(smiles1))
            continue
        try:
            molgraph = graph_from_smiles(smiles2)
            molgraph.sort_nodes_by_degree('atom')
            arrayrep = array_rep_from_smiles(molgraph)

            smiles_to_fingerprint_array[smiles1] = arrayrep
...
def save_smiles_dicts(smilesList, max_atom_len=0, max_bond_len=0):
    #first need to get the max atom length
    num_atom_features = 0
    num_bond_features = 0
    smiles_to_rdkit_list = {}
